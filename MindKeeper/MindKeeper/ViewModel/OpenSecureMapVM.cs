...
{
    public class OpenSecureMapVM : ViewModelBase
    {
        #region FIELDS
        ...
        private void ExecuteChoosePathCommand(object parameter)
        {
            ...
            if (PathToFile.Length != 0 && PathToFile.Substring(PathToFile.Length - 4, 4) == "smmp")
                IsProtected = true;
            else
                IsProtected = false;
        }
        ...
        public DelegateCommand OpenCommand
        {
            get
            {
                if (_openCommand == null) _openCommand = new DelegateCommand(ExecuteOpenCommand, CanExecuteOpenCommand);
                return _openCommand;
            }
        }
        ...
        private void ExecuteOpenCommand(object parameter)
        {
            ...
            if (!_isFailed)
            {
                ...
                Map map = worker.DecryptAndDeserialize();
                System.Windows.Forms.MessageBox.Show(map.Name + " deserialized");
                GeneralVm.ActiveMap = map;
            }
        }
